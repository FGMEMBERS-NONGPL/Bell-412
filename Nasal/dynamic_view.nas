...
view_manager.default_plane = func {
	...
	me.roll_offset = wow * rand() * speed * taxi_factor;	# MHM : taxing	
	
	if(reverser_act)	# MHM : reversethrust induced shake
		me.roll_offset = me.roll_offset + rand() *  engineittN.getValue() * reversethrust_factor; # mhm: optimizar
	...
	var u = me.vx.get() - wspd * cos(wdir);
	var slip = sin(me.slipN.getValue()) * me.ubody.filter(normatan(u / 10));

	var roll_shake = me.roll_offset * 0.1;
	
	me.heading_offset =								# view heading
		-1 * roll_factor * sin(me.roll) * cos(me.pitch)			#     due to roll
		+ groundsteering_factor * steering * wow	#     due to ground steering
		+ sideslip_factor * slip * (1 - wow)		#     due to sideslip (in air)
		+ roll_shake;

	me.pitch_offset =									# view pitch
		rollpitch_factor * sin(me.roll) * sin(me.roll)				#     due to roll
		+ gloadpitch_factor * (1 / (1 + math.exp( 2 - me.az.get() ))	#     due to G load
...
#begin mhm
var roll_factor = 0;
var groundsteering_factor = 0;
var sideslip_factor = 0;
var rollpitch_factor = 0;
var gloadpitch_factor = 0;
var speedMachN = nil;
var engineittN = nil;
var flaps_value = 0.0;
var speedbrake_act = 0;
var reverser_act = 0;
var taxi_factor = 0.0;
var reversethrust_factor = 0.0;
var flaps_factor = 0.0;
var speedbrake_factor = 0.0;
#end mhm
...
_setlistener("/sim/signals/nasal-dir-initialized", func {
	...
	#begin MHM
	props.globals.initNode("/sim/current-view/dynamic-view/parameters/roll-factor", 15, "INT");	
	setlistener("/sim/current-view/dynamic-view/parameters/roll-factor",func(n) { roll_factor = n.getValue() });
	props.globals.initNode("/sim/current-view/dynamic-view/parameters/groundsteering-factor", 10, "INT");	
	setlistener("/sim/current-view/dynamic-view/parameters/groundsteering-factor",func(n) { groundsteering_factor = n.getValue() });
	props.globals.initNode("/sim/current-view/dynamic-view/parameters/sideslip-factor", 10, "INT");
	setlistener("/sim/current-view/dynamic-view/parameters/sideslip-factor",func(n) { sideslip_factor = n.getValue() });
	props.globals.initNode("/sim/current-view/dynamic-view/parameters/rollpitch-factor", 10, "INT");
	setlistener("/sim/current-view/dynamic-view/parameters/rollpitch-factor",func(n) { rollpitch_factor = n.getValue() });
	props.globals.initNode("/sim/current-view/dynamic-view/parameters/gloadpitch-factor", 30, "INT");
	setlistener("/sim/current-view/dynamic-view/parameters/gloadpitch-factor",func(n) { gloadpitch_factor = n.getValue() });
	props.globals.initNode("/sim/current-view/shake-effect/parameters/taxi-factor", 0.5, "DOUBLE");
	setlistener("/sim/current-view/shake-effect/parameters/taxi-factor",func(n) { taxi_factor = n.getValue() }); 	    	
	props.globals.initNode("/sim/current-view/shake-effect/parameters/flaps-factor", 0.5, "DOUBLE");
	setlistener("/sim/current-view/shake-effect/parameters/flaps-factor",func(n) { flaps_factor = n.getValue() }); 	    	
	props.globals.initNode("/sim/current-view/shake-effect/parameters/speedbrake-factor", 0.55, "DOUBLE");
	setlistener("/sim/current-view/shake-effect/parameters/speedbrake-factor",func(n) { speedbrake_factor = n.getValue() }); 	    	
	props.globals.initNode("/sim/current-view/shake-effect/parameters/reversethrust-factor", 0.25, "DOUBLE");
	setlistener("/sim/current-view/shake-effect/parameters/reversethrust-factor",func(n) { reversethrust_factor = n.getValue() }); 	    	
	setlistener("/controls/flight/flaps",func(n) { flaps_value = n.getValue() });
	setlistener("/controls/engines/engine/reverser",func(n) { reverser_act = n.getValue() });
	setlistener("/controls/flight/speedbrake",func(n) { speedbrake_act = n.getValue() });
	speedMachN = props.globals.getNode("/velocities/mach", 1);
	engineittN = props.globals.getNode("/engines/engine/itt-norm", 1);
	flaps_value = getprop("/controls/flight/flaps");
	speedbrake_act = getprop("/controls/flight/speedbrake");
	reverser_act = getprop("/controls/engines/engine/reverser");
	#
